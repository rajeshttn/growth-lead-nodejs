...
module.exports = function (config) {
    ...
    self.get = function (key, cb) {
        ...
        if (!cache[key]) {
            memcached.get(key, function (err, data) {
                //console.log("memcached Get Called ", key, err, data);
                //if (err) {
    ...
    self.set = function (key, value, ttl, cb) {
        ...
        memcached.set(key, value, ttl, function (err) {
            //console.log("memcached Set Called ", key, err, value);
            //if (err) {
    ...
    self.clear = function (key, cb) {
        console.log('here inside clear cache', key);
        if (cache[key]) {
            console.log('clear local cache', key);
            delete cache[key];
        }

        memcached.del(key, function (err, data) {
            //if (err) {
            //    console.log("del Memcache Error ::", err);
            //}
            if (err) throw err;
            delete cache[key];
            if (cb) cb();
